$(function(){
    ...
    $('dl[spec_type="date"]').on('click', 'input[type="checkbox"]', function(){
        pv = $(this).parents('li').find('span[nctype="pv_name"]');
        if(typeof(pv.find('input').val()) == 'undefined'){
            pv.html('<input type="text" maxlength="20" class="text" value="'+pv.html()+'" />');
        }else{
        ...
        $(this).parents('li').find('input[type="text"]').datepicker({dateFormat: 'yy-mm-dd', minDate: new Date()});        
    }); 
 
    $('dl[spec_type="nondate"]').on('click', 'input[type="checkbox"]', function(){
        pv = $(this).parents('li').find('span[nctype="pv_name"]');
        if(typeof(pv.find('input').val()) == 'undefined'){
            pv.html('<input type="text" maxlength="20" class="text" value="'+pv.html()+'" />');
        }else{
    ...
    $('a[nctype="specDateAdd"]').click(function(){
        var _parent = $(this).parents('li:first');
        _parent.find('div[nctype="specAdd1"]').hide();
        _parent.find('div[nctype="specAdd2"]').show();
	    _parent.find('input[type="text"]').datepicker({dateFormat: 'yy-mm-dd', minDate: new Date()});
        _parent.find('input').focus();
    });
    // 取消
	...
	// 添加日期
	$('a[nctype="dateAdd"]').click(function(){
        var _parent = $(this).parent();		
        $('<span nctype="dateAdd2"><input class="text w100" type="text" name="data_list[]" maxlength="20"><a href="javascript:void(0);" nctype="dateDel" class="ncsc-btn ncsc-btn-orange">删除</span>').find('a').click(function(){
            $(this).parent().remove();
        }).end().insertBefore(_parent);
    });
	
	$('a[nctype="dateDel"]').click(function(){
        var _parent = $(this).parent();		
        _parent.remove();
    });
