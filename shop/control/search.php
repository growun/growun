...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if ($_GET['cate_name']){
            $cate = Model("goods_class")->where(array("gc_name" => $_GET['cate_name']))->find();
            if ($cate){
                $default_classid = $cate['gc_id'];   
                $_GET['cate_id'] = $cate['gc_id'];
            }
        }
        ...
        if ($_GET['keyword'] == '') {
            ...
            if (is_numeric($default_classid) && empty($_GET['keyword'])) {
                $seo_info = $model_goods_class->getKeyWords($default_classid);
                if (empty($seo_info[1])) {
        ...
        // 当前位置导航
        $nav_link_list = $model_goods_class->getGoodsClassNav($default_classid);
