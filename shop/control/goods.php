...
class goodsControl extends BaseGoodsControl {
    ...
    public function indexOp() {
        ...
        $actions = Model('action')->where(array('goods_id' => $goods_id))->select();
        ...
        $cate = Model("goods_class")->where(array("gc_id" => $goods_info['gc_id']))->find();
        Model('seo')->type('product')->param($seo_param)->show();
        if ($cate){
            if ($cate['gc_name'] == '童子军游学训练'){
                Tpl::showpage('goods2');    
            }else{
                Tpl::showpage('goods');
            }
