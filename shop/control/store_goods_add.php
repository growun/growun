...
class store_goods_addControl extends BaseSellerControl {
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $value_keys = array_keys($value['sp_value']);
                        $date_key = $value_keys[0];
                        $goods['date'] = strtotime($value['sp_value'][$date_key].' 12:00:00');
		    ...
		    $PhpQRCode->set('date',WAP_SITE_URL . '/tmpl/product_detail.html?goods_id='.$goods_id);
                ...
                redirect(urlShop('store_goods_add', 'add_action', array('goods_id' => $goods_id, 'commonid' => $common_id)));
            } else {
    ...
    public function add_actionOp(){
        $commonid = $_GET['commonid'];
        $goods_id = $_GET['goods_id'];
        $actions = Model('action')->where(array('goods_id' => $goods_id))->order('sort')->select();
       
        for( $i=0; $i<count($actions); $i++){
            $article = Model('cms_article')->where(array('action_id' => $actions[$i]['action_id']))->find();
            if(empty($article)){    
                $actions[$i]['article_id'] = null;
            }else{
                $actions[$i]['article_id'] = $article['article_id'];
            }
        }    
        Tpl::output('actions', $actions);
        Tpl::output('goods_common_id', $commonid);
        Tpl::output('goods_id', $goods_id);
        Tpl::showpage('store_action.add');
    }
