...
class indexControl extends BaseHomeControl{
	public function indexOp(){
            ...
            $cate = Model("goods_class")->where(array("gc_name" => "童子军游学训练"))->find();
            if ($cate){
                $default_classid = $cate['gc_id'];   
            }
              
        if ($_GET['start'] && $_GET['end'] ){
            $condition=array();
            $condition['date'] = array('egt', strtotime($_GET['start'].' 11:00:00'));
            $condition['date'] = array('elt', strtotime($_GET['end'].' 13:00:00'));
            $goods_list = Model('goods')->getGoodsList($condition);
            $output_arrays = array();
            ...
            foreach ($goods_list as $k => $v){
                ...
                $arr = explode(" ",$title);
                $title = $arr[0];
                $goods_array['title'] = $title;
                
                $goods_array['start'] = date("Y-m-d", $v['date']);
                $goods_array['url'] = urlShop('goods','index',array('goods_id'=>$v['goods_id']));
                $output_arrays[] = $goods_array;
            }
...
//友情链接
        ...
        //$goods_list = Model('goods')->getGoodsList($order = 'goods_id desc', $page = 8, $limit = 8);
        
        $goods_list = Model('goods')->where(array("gc_id" => $default_classid))->order("goods_id desc")->limit(8)->select();
        //print_r ($goods_list);
        //return;
		
		for ($i= 0;$i< count($goods_list); $i++){ 
            $goods_list[$i]['store_info'] = Model("store")->where(array('store_id' => $goods_list[$i]['store_id']))->find();
        } 
    ...
    public function get_calenderOp() {
