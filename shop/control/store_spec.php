...
class store_specControl extends BaseSellerControl {
    ...
    public function save_actionOp(){
        ...
        if (is_array($_POST['sv']['old'])) {
            ...
            foreach ($_POST['sv']['old'] as $key=> $value) {
                ...
                if (empty($value['action_title'])) {
                    continue;
                }
                $where = array('action_id' => $key);
                $update = array(
                        'action_title' => $value['action_title'],
                        'sort' => intval($value['sort']),
                        'action_type' => intval($value['action_type']),
                        'day' => intval($value['day']),
                        //'goods_common_id' => intval($_POST['goods_common_id'])，
                        'goods_id' => intval($_POST['goods_id'])
                    );
                
                $model = Model();
                
                Model('action')->where($where)->update($update);
        ...
        if (is_array($_POST['sv']['new'])) {
            ...
            $model_action->insertAll($insert_array);
    ...
    public function ajax_delactionOp(){
        $sp_value_id = intval($_GET['id']);
        if ($sp_value_id <= 0) {
            echo 'false'; exit();
        }
        $rs = Model('action')->where(array('action_id' => $sp_value_id))->delete();
        if ($rs) {
            echo 'true'; exit();
        } else {
            echo 'false'; exit();
